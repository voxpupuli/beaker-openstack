...
A quick acceptance test, named because it has no pre-suites to run
    ...
    task :quick do
      ...
      unless ENV['OPENSTACK_HOSTS'] and ENV['OPENSTACK_KEY']
        raise(ArgumentError, "Please set OPENSTACK_HOSTS and OPENSTACK_KEY environment variables")
      end

      sh("beaker",
         "--hosts", ENV['OPENSTACK_HOSTS'],
         "--tests", beaker_test_base_dir,
         "--log-level", "debug",
         "--load-path", load_path_option,
         "--key", ENV['OPENSTACK_KEY'],
         "--debug")
    end
