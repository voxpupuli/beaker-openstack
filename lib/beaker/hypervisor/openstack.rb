module Beaker
  ...
  class Openstack < Beaker::Hypervisor
    ...
    # @option options [String] :openstack_tenant Tenant (v2) or nil
    ...
    def initialize(openstack_hosts, options)
      ...
      @vms     = []
      @ephemeral_keypairs = []  # Track keys we create so we only delete what we own
      ...
      if is_v3
        ...
        # tenant invalid under v3
        raise 'Invalid option: v3 API does not use :openstack_tenant' if @options[:openstack_tenant]
        ...
        # v2: tenant required
        raise 'Specify openstack_tenant for v2 auth' unless @options[:openstack_tenant]

        # v2: project fields invalid
        if @options[:openstack_project_name] || @options[:openstack_project_id]
          raise 'Invalid option: v2 API does not use :openstack_project_name or :openstack_project_id'
        end
      end
      ...
      extra_credentials =
        ...
        elsif @options[:openstack_project_id]
    ...
    def create_instance_resources(host)
      ...
      vm.wait_for(@options[:timeout] || 600) { ready? }
